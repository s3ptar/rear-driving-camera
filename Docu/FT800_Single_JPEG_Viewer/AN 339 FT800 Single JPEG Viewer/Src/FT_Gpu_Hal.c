...
//intialise and return info about MPSSE cable
//**************************************************************************************
...
//***************************************************************************************
{
	...
	/* By default it is assumed that only one mpsse cable is connected to PC and channel 0 of that mpsse cable is used for spi transactions */
...
//*********************************************************************************************************
ft_void_t  Ft_Gpu_Hal_StartTransferB(Ft_Gpu_Hal_Context_t *host,FT_GPU_TRANSFERDIR_T rw,ft_uint32_t addr)
//*********************************************************************************************************
...
//******************************************************************************
{
	...
	if (host->status == FT_GPU_HAL_WRITING){
		Ft_Gpu_Hal_Transfer16(host,value & 0xFFFF);
...
#define MAX_CMD_FIFO_TRANSFER   Ft_Gpu_Cmdfifo_Freespace(host)  //check freespace in FIFO.
	do {                
		length = count;
		...
		if (length > MAX_CMD_FIFO_TRANSFER){
		    length = MAX_CMD_FIFO_TRANSFER;
      	        ...
      	        Ft_Gpu_Hal_CheckCmdBuffer(host,length);  //check freespace
		...
		Ft_Gpu_Hal_Updatecmdfifo(host,length); //update REG_CMD_WRITE with last valid address for data
...
{
	//check if all commands have been completed  REG_CMD_READ updated by copro, REG_CMD_WRITE by MCU
...
//**********************************************************************
//write 4 bytes into Command buffer
{
